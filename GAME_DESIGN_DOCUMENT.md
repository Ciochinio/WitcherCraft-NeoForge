...
Signposts are tall wooden posts that work as fast-travel destinations. Each one appears on the world map as
`Signpost: <name>`.
...
A signpost shows up on the map as a question mark once you come within **128** blocks, and you discover it when you
get within **16** blocks. By default every player discovers signposts on their own. Worlds can turn on shared
discovery instead: once anyone finds a signpost, everyone has it.

**Placing your own**

A signpost needs two blocks of free space. After placing it you are asked to name it (up to **64** characters). If
you skip naming, it is named after its coordinates, for example `Signpost: 120, -45`. You discover your own
signpost straight away, and every player sees the same name. Each world allows up to **256** player-placed
signposts; village signposts do not count toward this.

**Breaking a signpost**

Breaking either half, or blowing it up, removes the signpost and its map marker for everyone. It drops as an item
unless the world turns that off. A signpost placed again counts as a new destination and has to be discovered
again.
...
|---------|---------|--------|
| Fast Travel Enabled | **on** | Turns signposts and travel on or off. Turning it off hides signposts from the map without deleting anything. Needs a world restart. |
