...
import net.redboltmedia.witchercraft.network.GraveirGuiButtonMessage;
...
public class GraveirGuiScreen extends AbstractContainerScreen<GraveirGuiMenu> implements WitchercraftModScreens.ScreenAccessor {
	...
	public GraveirGuiScreen(GraveirGuiMenu container, Inventory inventory, Component text) {
	...
	protected void extractLabels(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
		guiGraphics.text(this.font, Component.translatable("gui.witchercraft.graveir_gui.label_drowner"), 17, -11, -12829636, false);
		guiGraphics.text(this.font, Component.translatable("gui.witchercraft.graveir_gui.label_type_hostal"), 17, 7, -12829636, false);
		guiGraphics.text(this.font, Component.translatable("gui.witchercraft.graveir_gui.label_health_20"), 17, 34, -12829636, false);
		guiGraphics.text(this.font, Component.translatable("gui.witchercraft.graveir_gui.label_atack_damage_5"), 17, 61, -12829636, false);
	}
	...
	public void init() {
		...
		imagebutton_arrowforword = new ImageButton(this.leftPos + 206, this.topPos + 124, 32, 32,
				new WidgetSprites(Identifier.parse("witchercraft:textures/screens/arrowforword.png"), Identifier.parse("witchercraft:textures/screens/arrowforword.png")), e -> {
					int x = GraveirGuiScreen.this.x;
					int y = GraveirGuiScreen.this.y;
					if (true) {
						ClientPacketDistributor.sendToServer(new GraveirGuiButtonMessage(0, x, y, z));
						GraveirGuiButtonMessage.handleButtonAction(entity, 0, x, y, z);
					}
				}) {
			@Override
		...
		imagebutton_bookmark = new ImageButton(this.leftPos + 204, this.topPos + -23, 32, 32, new WidgetSprites(Identifier.parse("witchercraft:textures/screens/bookmark.png"), Identifier.parse("witchercraft:textures/screens/bookmark.png")), e -> {
			...
			int y = GraveirGuiScreen.this.y;
			if (true) {
				ClientPacketDistributor.sendToServer(new GraveirGuiButtonMessage(1, x, y, z));
				GraveirGuiButtonMessage.handleButtonAction(entity, 1, x, y, z);
			}
		}) {
			@Override
			public void extractContents(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		imagebutton_bookmarkleft = new ImageButton(this.leftPos + -28, this.topPos + 34, 60, 23,
				new WidgetSprites(Identifier.parse("witchercraft:textures/screens/bookmarkleft.png"), Identifier.parse("witchercraft:textures/screens/bookmarkleft.png")), e -> {
				}) {
			@Override
			public void extractContents(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
				guiGraphics.blit(RenderPipelines.GUI_TEXTURED, sprites.get(isActive(), isHoveredOrFocused()), getX(), getY(), 0, 0, width, height, width, height);
		...
		imagebutton_bookmarkerleftbought = new ImageButton(this.leftPos + -28, this.topPos + 7, 60, 23,
		...
		imagebutton_arrowback = new ImageButton(this.leftPos + 17, this.topPos + 124, 32, 32, new WidgetSprites(Identifier.parse("witchercraft:textures/screens/arrowback.png"), Identifier.parse("witchercraft:textures/screens/arrowback.png")), e -> {
			...
			int y = GraveirGuiScreen.this.y;
			if (true) {
				ClientPacketDistributor.sendToServer(new GraveirGuiButtonMessage(5, x, y, z));
				GraveirGuiButtonMessage.handleButtonAction(entity, 5, x, y, z);
			}
		}) {
			@Override
			public void extractContents(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
