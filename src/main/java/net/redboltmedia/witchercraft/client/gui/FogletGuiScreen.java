...
import net.redboltmedia.witchercraft.world.inventory.FogletGuiMenu;
import net.redboltmedia.witchercraft.network.FogletGuiButtonMessage;
...
public class FogletGuiScreen extends AbstractContainerScreen<FogletGuiMenu> implements WitchercraftModScreens.ScreenAccessor {
	...
	public FogletGuiScreen(FogletGuiMenu container, Inventory inventory, Component text) {
	...
	protected void extractLabels(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
		guiGraphics.text(this.font, Component.translatable("gui.witchercraft.foglet_gui.label_drowner"), 17, -11, -12829636, false);
		guiGraphics.text(this.font, Component.translatable("gui.witchercraft.foglet_gui.label_type_hostal"), 17, 7, -12829636, false);
		guiGraphics.text(this.font, Component.translatable("gui.witchercraft.foglet_gui.label_health_20"), 17, 34, -12829636, false);
		guiGraphics.text(this.font, Component.translatable("gui.witchercraft.foglet_gui.label_atack_damage_5"), 17, 61, -12829636, false);
	}
	...
	public void init() {
		...
		imagebutton_arrowforword = new ImageButton(this.leftPos + 206, this.topPos + 124, 32, 32,
				new WidgetSprites(Identifier.parse("witchercraft:textures/screens/arrowforword.png"), Identifier.parse("witchercraft:textures/screens/arrowforword.png")), e -> {
					int x = FogletGuiScreen.this.x;
					int y = FogletGuiScreen.this.y;
					if (true) {
						ClientPacketDistributor.sendToServer(new FogletGuiButtonMessage(0, x, y, z));
						FogletGuiButtonMessage.handleButtonAction(entity, 0, x, y, z);
					}
				}) {
			@Override
		...
		imagebutton_bookmark = new ImageButton(this.leftPos + 204, this.topPos + -23, 32, 32, new WidgetSprites(Identifier.parse("witchercraft:textures/screens/bookmark.png"), Identifier.parse("witchercraft:textures/screens/bookmark.png")), e -> {
			...
			int y = FogletGuiScreen.this.y;
			if (true) {
				ClientPacketDistributor.sendToServer(new FogletGuiButtonMessage(1, x, y, z));
				FogletGuiButtonMessage.handleButtonAction(entity, 1, x, y, z);
			}
		}) {
			@Override
			public void extractContents(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		imagebutton_arrowback = new ImageButton(this.leftPos + 17, this.topPos + 124, 32, 32, new WidgetSprites(Identifier.parse("witchercraft:textures/screens/arrowback.png"), Identifier.parse("witchercraft:textures/screens/arrowback.png")), e -> {
			...
			int y = FogletGuiScreen.this.y;
			if (true) {
				ClientPacketDistributor.sendToServer(new FogletGuiButtonMessage(5, x, y, z));
				FogletGuiButtonMessage.handleButtonAction(entity, 5, x, y, z);
			}
		}) {
			@Override
			public void extractContents(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
