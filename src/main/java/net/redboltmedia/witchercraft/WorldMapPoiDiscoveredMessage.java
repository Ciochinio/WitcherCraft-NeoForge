...
@EventBusSubscriber
public record WorldMapPoiDiscoveredMessage(String translationKey, String fallbackName) implements CustomPacketPayload {
	...
	public static final StreamCodec<RegistryFriendlyByteBuf, WorldMapPoiDiscoveredMessage> STREAM_CODEC = StreamCodec.of(
		(buffer, message) -> { buffer.writeUtf(message.translationKey, MAX_TEXT); buffer.writeUtf(message.fallbackName, MAX_TEXT); },
		buffer -> new WorldMapPoiDiscoveredMessage(buffer.readUtf(MAX_TEXT), buffer.readUtf(MAX_TEXT)));
		...
		if (context.flow() == PacketFlow.CLIENTBOUND)
			context.enqueueWork(() -> {
				if (WorldMapClientConfig.discoveryActionBar() && Minecraft.getInstance().player != null) {
					Component name = Component.translatableWithFallback(message.translationKey, message.fallbackName);
