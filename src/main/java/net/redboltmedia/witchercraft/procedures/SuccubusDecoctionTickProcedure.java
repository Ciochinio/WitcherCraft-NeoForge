...
public class SuccubusDecoctionTickProcedure {
	public static void execute(Entity entity) {
		...
		if (entity instanceof LivingEntity _livingEntity0 && _livingEntity0.getAttributes().hasAttribute(WitchercraftModAttributes.INCREASED_DAMAGE)) {
			if ((entity instanceof LivingEntity _livEnt && _livEnt.hasEffect(WitchercraftModMobEffects.SUCCUBUS_DECOCTION_EFFECT) ? _livEnt.getEffect(WitchercraftModMobEffects.SUCCUBUS_DECOCTION_EFFECT).getDuration() : 0) % 40 == 0) {
				...
				if (entity instanceof LivingEntity _entity) {
					AttributeModifier modifier = new AttributeModifier(Identifier.parse("witchercraft:effect_succubus_stacks"), entity.getData(WitchercraftModVariables.PLAYER_VARIABLES).witchercraftSuccubusDecoctionTick,
