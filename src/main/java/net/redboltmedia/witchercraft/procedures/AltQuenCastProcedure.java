...
public class AltQuenCastProcedure {
	public static void execute(Entity entity) {
		...
		if (entity.getData(WitchercraftModVariables.PLAYER_VARIABLES).witchercraftSignKeyHoldTime % 20 == 0) {
			{
				WitchercraftModVariables.PlayerVariables _vars = entity.getData(WitchercraftModVariables.PLAYER_VARIABLES);
				_vars.witchercraftQuenShield = entity.getData(WitchercraftModVariables.PLAYER_VARIABLES).witchercraftQuenShield + 2 * (1
						+ (entity instanceof LivingEntity _livingEntity4 && _livingEntity4.getAttributes().hasAttribute(WitchercraftModAttributes.SIGN_INTENSITY) ? _livingEntity4.getAttribute(WitchercraftModAttributes.SIGN_INTENSITY).getValue() : 0)
								* 0.01);
				_vars.markSyncDirty();
			}
		}
