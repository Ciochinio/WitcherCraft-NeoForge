		...
		if (entity.isAlive()) {
			{
				...
				_vars.witchercraftStaminaRegenBuffer = entity.getData(WitchercraftModVariables.PLAYER_VARIABLES).witchercraftStaminaRegenBuffer
						+ (entity instanceof LivingEntity _livEntR && _livEntR.getAttributes().hasAttribute(WitchercraftModAttributes.PASSIVE_STAMINA_REGEN)
								? _livEntR.getAttribute(WitchercraftModAttributes.PASSIVE_STAMINA_REGEN).getValue()
								: 0);
				_vars.markSyncDirty();
			}
