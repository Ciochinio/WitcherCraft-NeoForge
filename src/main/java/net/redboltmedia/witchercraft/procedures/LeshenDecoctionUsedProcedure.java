...
public class LeshenDecoctionUsedProcedure {
	public static void execute(Entity entity) {
		...
		if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
			_entity.addEffect(new MobEffectInstance(WitchercraftModMobEffects.LESHEN_DECOCTION_EFFECT, (int) (7200 * (1 + (entity instanceof LivingEntity _livingEntityPD && _livingEntityPD.getAttributes().hasAttribute(WitchercraftModAttributes.POTION_DURATION) ? _livingEntityPD.getAttribute(WitchercraftModAttributes.POTION_DURATION).getValue() : 0) * 0.01)), 0));
		{
